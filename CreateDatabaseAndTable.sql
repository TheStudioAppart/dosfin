...
CREATE TABLE IF NOT EXISTS `TheStudioAppart`.`TC_USER` (
  ...
  INDEX `groupUser_idx` (`GroupId` ASC) VISIBLE,
  INDEX `AddressUser_idx` (`AddressId` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `TheStudioAppart`.`T_Event` (
  ...
  `CTTId` INT NULL,
  PRIMARY KEY (`Id`),
  INDEX `typeEvent_idx` (`TypeEventId` ASC) VISIBLE,
  INDEX `statusEvent_idx` (`StatusId` ASC) VISIBLE,
  INDEX `addressEvent_idx` (`AddressId` ASC) VISIBLE,
  INDEX `CttEvent_idx` (`CTTId` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `TheStudioAppart`.`T_UserInvited` (
  ...
  `EventId` INT NULL,
  PRIMARY KEY (`Id`),
  INDEX `eventInvited_idx` (`EventId` ASC) VISIBLE,
  INDEX `userInvited_idx` (`UserId` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `TheStudioAppart`.`T_EventParticipant` (
  ...
  `EventId` INT NULL,
  PRIMARY KEY (`Id`),
  INDEX `eventParticipantevent_idx` (`EventId` ASC) VISIBLE,
  INDEX `eventParticipantUser_idx` (`UserId` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `TheStudioAppart`.`T_CTTParticipant` (
  ...
  PRIMARY KEY (`Id`),
  INDEX `eventParticipantUser_idx` (`UserId` ASC) VISIBLE,
  INDEX `cttParticipantctt_idx` (`CTTId` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `TheStudioAppart`.`T_ListAchat` (
  ...
  `CttId` INT NOT NULL,
  PRIMARY KEY (`Id`),
  INDEX `cttList_idx` (`CttId` ASC) VISIBLE,
  CONSTRAINT `cttList`
    FOREIGN KEY (`CttId`)
    REFERENCES `TheStudioAppart`.`T_CTT` (`Id`)
    ON DELETE NO ACTION
...
CREATE TABLE IF NOT EXISTS `TheStudioAppart`.`T_ListAchatItem` (
  ...
  `Quantite` INT NULL,
  PRIMARY KEY (`Id`),
  INDEX `ListAchat_idx` (`ListAchatId` ASC) VISIBLE,
  INDEX `produit_idx` (`ProduitId` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `TheStudioAppart`.`T_Depense` (
  ...
  PRIMARY KEY (`Id`),
  INDEX `cttdepense_idx` (`CttId` ASC) VISIBLE,
  INDEX `listachatitem_idx` (`AchaItemId` ASC) VISIBLE,
  INDEX `listachat_idx` (`ListAchatId` ASC) VISIBLE,
  INDEX `acheteur_idx` (`AcheteurId` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `TheStudioAppart`.`T_DepenseParticipant` (
  ...
  PRIMARY KEY (`Id`),
  INDEX `depenseparticipant_idx` (`ParticipantId` ASC) VISIBLE,
  INDEX `depensedepense_idx` (`DepenseId` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `TheStudioAppart`.`T_StockItem` (
  ...
  PRIMARY KEY (`Id`),
  INDEX `stockitemproduit_idx` (`ProduitId` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `TheStudioAppart`.`TS_Stock` (
  ...
  PRIMARY KEY (`Id`),
  INDEX `Stock_idx` (`StockId` ASC) VISIBLE,
  CONSTRAINT `Stock`
    FOREIGN KEY (`StockId`)
